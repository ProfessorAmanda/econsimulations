...
export default function ChartContainer({ popArray, popMean, sampled, popType, sampleSize }) {
  ...
  useEffect(() => {
    ...
    const newChart = Highcharts.chart('container', {
      ...
      title: {
        text: `${values[popType].title} <br /> Population Mean: ${Math.round(popMean * 100) / 100}`
      },
      yAxis: {
        max: ymaxval,
        startOnTick: true,
  ...
  // TODO: figure out how to plot the sampled points
