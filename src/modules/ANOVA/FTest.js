  ...
  return (
    <Table borderless style={{marginTop: 20}}>
      <thead>
        <tr>
          <th/>
          <th><TeX math={`\\bar{\\bar{x}} = ${_.round(overallSampleMean, 2)}`} block/></th>
          <th/>
          <th/>
        </tr>
        <tr>
      ...
      <tbody>
        ...
        <tr>
          <td/>
          <td/>
          <td><TeX math={`F = ${F.toPrecision(3)}`} block/></td>
          <td><TeX math={`p-value \\approx ${pValue.toPrecision(3)}`} block/></td>
        </tr>
      </tbody>
    </Table>
  )
