      ...
      <Row md={1} lg={2}>
        ...
        <Col>
          <div style={{ marginLeft: '20%' }}>
            <TeX block math={`\\widehat{${(regressorType === 'Continuous') ? 'Test\\ Score' : 'Earnings'}}_i = \\hat{\\beta}_0 + \\hat{\\beta}_1{${(regressorType === 'Continuous') ? 'Study\\ Hours' : 'Job\\ Corps'}_i}`}/>
            {selected && (
              <TeX block math={`\\widehat{${(regressorType === 'Continuous') ? 'Test\\ Score' : 'Earnings'}}_i = ${selected.intercept} + ${selected.slope}{${(regressorType === 'Continuous') ? 'Study\\ Hours' : 'Job\\ Corps'}_i}`}/>
            )}
          </div>
          <ScatterPlot
            series={sampleSeries}
