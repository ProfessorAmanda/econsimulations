...
export default function Conclusion({ firstMean, secondMean, equality, reject, testType }) {
  ...
  return (
    (testType === 'oneSample') ? (
      <p>The true mean is {relation} <TeX math="\mu_0"/>. Therefore we should {(result) ? 'reject' : 'fail to reject'} the null hypothesis. Our conclusion above was {(result === reject) ? 'correct' : 'incorrect'}</p>
    ) : (
      <p>
